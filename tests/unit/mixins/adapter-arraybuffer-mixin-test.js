...
module('Unit | Mixin | adapter arraybuffer mixin', function () {
  ...
  test('it does not do anything if no options is given', function (assert) {
    assert.deepEqual(
      createSubject().ajaxOptions('http://domain.com', 'GET', { key: 'value' }),
      {
        url: 'http://domain.com',
        type: 'GET',
        options: {
  ...
  test('it sets dataType to arraybuffer and processData to false if arraybuffer:true', function (assert) {
    assert.deepEqual(
      createSubject().ajaxOptions('http://domain.com', 'GET', {
        arraybuffer: true,
      }),
      {
        url: 'http://domain.com',
  ...
  test('it sets dataType to blob and processData to false if blob:true', function (assert) {
    assert.deepEqual(
      createSubject().ajaxOptions('http://domain.com', 'GET', { blob: true }),
      {
        url: 'http://domain.com',
        type: 'GET',
        dataType: 'blob',
